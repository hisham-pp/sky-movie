...
const latestRelease = releaseManifest.releases.find((release) => release.version === releaseManifest.latestVersion) ?? releaseManifest.releases[0];

const downloadPlatforms: Array<{ platform: ReleasePlatform; label: string; detail: string }> = [
  { platform: 'windows', label: 'Windows', detail: 'NSIS setup installer' },
  { platform: 'macos', label: 'macOS', detail: 'DMG and ZIP builds' },
  { platform: 'linux', label: 'Linux', detail: 'AppImage, DEB, and tar.gz' }
];

function artifactsForPlatform(platform: ReleasePlatform) {
  return latestRelease?.artifacts.filter((artifact) => artifact.platform === platform) ?? [];
}

function primaryArtifact(platform: ReleasePlatform) {
    ...
    <>
      ...
      <main id="top">
        ...
        <section id="downloads" className="section">
          <div className="section-inner">
            <div className="section-heading">
              <h2>Download Sky Movie desktop.</h2>
              <p>
                Latest release metadata is generated by the Cloudflare R2 release script. Each release is uploaded under its own version prefix.
              </p>
            </div>

            <div className="download-panel">
              <div className="release-summary">
                <span className="release-eyebrow">Latest version</span>
                <h3>Sky Movie {latestRelease?.version ?? releaseManifest.latestVersion}</h3>
                <p>{latestRelease?.notes ?? 'Release artifacts have not been uploaded yet.'}</p>
                <div className="release-meta-grid">
                  <div>
                    <span>Released</span>
                    <strong>{formatReleaseDate(latestRelease?.releasedAt)}</strong>
                  </div>
                  <div>
                ...
                <div className="release-commit">
                  <span>Last released commit message</span>
                  <strong>{latestRelease?.sourceCommit?.message ?? 'Pending release upload'}</strong>
                </div>
                {(latestRelease?.storageFolderUrl ?? latestRelease?.driveFolderUrl) ? (
                  <a className="button light" href={latestRelease.storageFolderUrl ?? latestRelease.driveFolderUrl ?? '#'} target="_blank" rel="noopener noreferrer">
                    Open version prefix
                  </a>
                ) : null}
              ...
              <div className="download-grid">
                {downloadPlatforms.map((item) => {
                  const artifact = primaryArtifact(item.platform);
                  const artifacts = artifactsForPlatform(item.platform);
                  return (
                    <article className="download-card" key={item.platform}>
                      <div>
                        ...
                        <p>
                          {artifact
                            ? `${artifact.fileName} (${formatArtifactSize(artifact.size)})`
                            : 'Not uploaded for this release yet.'}
                        </p>
                      </div>
                      {artifact ? (
                        <a className="button primary" href={artifact.downloadUrl}>
                          Download {item.label}
                        </a>
                      ) : (
                      ...
                      {artifacts.length > 1 ? (
                        <div className="artifact-links">
                          {artifacts.slice(1).map((extra) => (
                            <a key={extra.r2Key ?? extra.driveFileId ?? extra.fileName} href={extra.downloadUrl}>
                              {extra.kind} {extra.arch}
                            </a>
                          ))}
                        </div>
                      ) : null}
